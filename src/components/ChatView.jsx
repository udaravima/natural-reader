...
export default function ChatView({
  ...
  const ingestFiles = async (fileList) => {
    ...
    if (dropped.length > 0) {
      showToast?.(
        "Audio attachments are not supported yet — coming soon.",
        4000,
      );
    }
  ...
  return (
    ...
    >
      ...
      {isDragging && (
        <div
          className={`absolute inset-0 z-20 pointer-events-none flex items-center justify-center backdrop-blur-sm ${darkMode ? "bg-blue-900/30" : "bg-blue-100/60"}`}
        >
          <div
            className={`px-6 py-4 rounded-2xl border-2 border-dashed ${theme.border} ${theme.bgSecondary} flex items-center gap-3 shadow-xl`}
          >
  ...
  return (
    <div className={`flex gap-3 ${isUser ? "flex-row-reverse" : ""}`}>
      ...
      >
        ...
        {hasAttachments && (
          <div
            className={`flex flex-wrap gap-2 ${isUser ? "justify-end" : ""}`}
          >
            {attachments.map((att) => (
              <AttachmentPreview
                key={att.id}
        ...
        {(hasContent || (isUser && !hasAttachments)) && (
          <div
            className={`px-4 py-3 rounded-2xl ${
              isUser
                ? darkMode
                  ? "bg-blue-600/30 text-white rounded-tr-sm"
          ...
          <div className="flex items-center gap-1">
            ...
            {!isUser && onDownloadAudio && (
              <button
                ...
                title={
                  isDownloading ? "Generating audio…" : "Download audio (.wav)"
                }
              >
                {isDownloading ? (
...
function MessageStatsDisclosure({ stats, theme, darkMode }) {
  ...
  return (
    ...
    <div
      className={`w-fit max-w-full rounded-md ${darkMode ? "bg-slate-800/30" : "bg-slate-100/60"}`}
    >
      <button
        onClick={() => setOpen((o) => !o)}
      ...
      {open && (
        <dl
          className={`grid grid-cols-[max-content_max-content] gap-x-3 gap-y-0.5 px-3 pb-2 text-[10px] ${theme.textSecondary}`}
        >
          {stats.model && (
            <>
          ...
          {stats.promptEvalCount != null && (
            <>
              ...
              <dd>
                {stats.promptEvalCount} tok in {fmtNs(stats.promptEvalNs)}
              </dd>
            </>
          )}
  ...
  const components = useMemo(
    () => ({
      ...
      code: ({ inline, children }) =>
        inline ? (
          <code
            className={`px-1 py-0.5 rounded text-xs font-mono ${darkMode ? "bg-slate-700/60" : "bg-slate-200"}`}
          >
            {children}
          </code>
        ) : (
      ...
      pre: ({ children }) => (
        <pre
          className={`my-2 p-3 rounded-lg overflow-x-auto text-xs ${darkMode ? "bg-slate-800/80 text-slate-100" : "bg-slate-900/90 text-slate-100"}`}
        >
          {children}
        </pre>
      ),
      ul: ({ children }) => (
        <ul className="list-disc pl-5 my-1 text-sm leading-relaxed">
          {children}
        </ul>
      ),
      ol: ({ children }) => (
        <ol className="list-decimal pl-5 my-1 text-sm leading-relaxed">
          {children}
        </ol>
      ),
      ...
      blockquote: ({ children }) => (
        <blockquote
          className={`border-l-2 pl-3 my-1 italic ${darkMode ? "border-slate-500 text-slate-300" : "border-slate-400 text-slate-700"}`}
        >
          {children}
        </blockquote>
      ),
      ...
      th: ({ children }) => (
        <th className="border border-slate-500/40 px-2 py-1 font-semibold">
          {children}
        </th>
      ),
      td: ({ children }) => (
...
function ThinkingDisclosure({ text, theme, darkMode, isStreamingNow }) {
  ...
  return (
    <div
      className={`max-w-full rounded-xl border ${theme.border} ${darkMode ? "bg-slate-800/40" : "bg-slate-100/70"} overflow-hidden`}
    >
      <button
      ...
      {open && (
        <div
          className={`px-3 pb-3 pt-0 text-[11px] italic leading-relaxed whitespace-pre-wrap break-words ${theme.textMuted}`}
        >
          {text}
        </div>
      )}
...
function ToolCallsDisclosure({ toolCalls, theme, darkMode }) {
  ...
  return (
    <div
      className={`w-fit max-w-full rounded-md ${darkMode ? "bg-cyan-500/10" : "bg-cyan-50"}`}
    >
      <button
        onClick={() => setOpen((o) => !o)}
      ...
      {open && (
        <ul
          className={`px-3 pb-2 text-[10px] ${theme.textSecondary} space-y-1`}
        >
          {toolCalls.map((tc, i) => {
            const argStr = tc.arguments
              ? Object.entries(tc.arguments)
                  .map(
                    ([k, v]) =>
                      `${k}=${typeof v === "string" ? `"${v.length > 60 ? v.slice(0, 60) + "…" : v}"` : v}`,
                  )
                  .join(", ")
              : "";
            ...
            const resultLabel = result.error
              ...
              : result.chunk_count != null
                ...
                : "ok";
            const resultSummaryText = result.summary_text
              ? result.summary_text
              : "";
            return (
              <li key={i} className="flex items-start gap-1.5">
                ...
                <span className="font-mono break-all">
                  <span className="font-bold">{tc.name}</span>({argStr}){" "}
                  <span className={theme.textMuted}>→ {resultLabel}</span><br />
...
export function DocContextChip({
  ...
  if (!ctx) return null;
  const label =
    ctx.kind === "selection" ? "Selection" : `Page ${ctx.page ?? "?"}`;
  ...
  return (
    ...
    >
      <FileText
        size={14}
        className={`shrink-0 mt-0.5 ${darkMode ? "text-purple-300" : "text-purple-600"}`}
      />
      <div className="flex-1 min-w-0">
        <p className={`font-bold truncate ${theme.text}`}>
          {label}{" "}
          <span className={`font-normal ${theme.textMuted}`}>· {file}</span>
        </p>
        {preview && (
          <p className={`text-[11px] mt-0.5 truncate ${theme.textSecondary}`}>
            {preview}
            {ctx.text.length > 120 ? "…" : ""}
          </p>
        )}
...
function EmptyState({ theme, darkMode, reachable, selectedModel }) {
  return (
    <div className="h-full flex flex-col items-center justify-center text-center gap-4 px-6">
      <div
        className={`w-20 h-20 ${theme.bgTertiary} rounded-2xl flex items-center justify-center ${theme.textMuted} border-2 border-dashed ${theme.border}`}
      >
