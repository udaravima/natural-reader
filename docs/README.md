...
| 6 | [IDENTITY_AND_ROLES.md](IDENTITY_AND_ROLES.md) | **Developers** | How Keycloak users map to app users: the two stores, the `(iss, sub)` join key, JIT provisioning branches, roles (app-owned, not IdP claims), credentials, create/delete propagation rules (live-verified) |
