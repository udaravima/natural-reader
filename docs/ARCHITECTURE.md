...
- **Library**: `LibraryPage` (`viewMode:'library'`, any authenticated active user): list/search documents with project + tag filters and share indicators; a project/tags picker on upload/register.
...
- **`docs.py`** — the RAG pipeline plus the **Library** surface. State machine: `registered → chunks_uploaded → indexing → indexed|failed`. The `doc_id` is the client's sha256 (regex-gated `^[0-9a-f]{64}$`) because it's interpolated into filesystem paths. Chunks upsert idempotently; indexing embeds in batches of 16 under a per-doc lock; search is pgvector cosine distance. Docling conversion (PDF → per-page markdown) lives here too. Read routes honor **`can_read`** (owner / project-member / grantee — a **404**, never a 403, for non-readers); writes stay owner-only. Read access for every read path is the single predicate in `server/auth/authz.py` (`readable_docs_where` + `readable_docs_params`): owner, grantee, or owner/member of any linked project. `GET /v1/docs` lists own/member/granted docs (`q`/project/tag filters); `PATCH /v1/docs/{id}` sets tags or (admin) reassigns owner — it no longer touches projects (`project_id` is a 422); project links go through `projects.py`'s `PUT`/`DELETE /v1/projects/{id}/docs/{doc_id}`.
- **`projects.py`** — owner-only project CRUD, project membership, and per-document read grants (idempotent), and project↔document links (doc owners add; doc or project owners remove). Backs the Library's sharing model (migrations `009`, `010`).
