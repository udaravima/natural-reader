...
2. **Persisted settings** (`usePersistedState`, localStorage key prefix `neural-pdf-`): `darkMode`, `volume`, `scale`, `playbackSpeed`, `selectedVoice`, `isLocalhost` (see [traps](#invariants--traps) — despite the name it means "use Kokoro TTS, not Web Speech"), `apiHost`/`apiPort`, `requestTimeout` (seconds), `unlimitedBatchTimeout`, `viewMode` (`'reader'|'chat'`), `inferenceSource` (`'server'|'local'`, default **server**), Ollama `ollamaHost`/`ollamaPort`/`selectedModel` (local mode only), `chatTtsMode`, `chatAutoTts`, per-model inference overrides (`inferenceByModel`), `distractionFree`, layout knobs.
...
| `useChatEngine` | Chat driver behind the **`callChat()`** seam (`src/lib/chatTransport.js`): in Server mode requests go through the authenticated gateway (`GET /v1/inference/models`, `POST /v1/inference/chat`); in Local mode they go direct to the user's Ollama (`/api/tags`, `/api/chat`) via the Vite proxy. NDJSON stream consumption, think-mode fallback chain, tool execution, chat TTS queue, session persistence, pins, event log, **429 interception before any retry chain** (budget-exhausted toast), budget state for the meter. | gateway `/v1/inference/*` or Ollama `/api/*`; sessions via sessionStore; tools via `/v1/...` |
...
- **`docMeta.js`** — `registerDocument` uploads a file's bytes multipart to `POST /v1/docs` (tags travel with it; an optional project link is a fail-soft follow-up request) and returns the server's authoritative `{docId, dedup, state}`. The single place both the index path and the convert path register a document, so the sequencing can't drift between the two call sites. The old client-side chunk upload (`uploadPdf.js`) is gone — the server derives chunks itself from the verified bytes.
- **`apiErrors.js`** — the one central refusal-to-notice mapper (`describeRefusal`/`noticeFor`): reads the `{error, message, reason?}` body FastAPI wraps refusals in and turns it into toast text, keyed mostly by HTTP status — 409 `content_shared` picks `other_holders`/`in_project` wording, 413/415/422(`empty_file`)/404 get fixed copy, 5xx is the only case that blames the server, and anything else falls back to the body's own `message`.
...
- **`docHash.js`** — WebCrypto sha256 of file bytes, memoized per name+size. Used to key IndexedDB and the per-document state maps, and sent as `client_doc_id` on upload — but it's only a **hint**: the server computes its own hash from the bytes it receives, and that value is the authoritative `doc_id` if the two ever disagree (a stale or lying client).
...
- **App shell**: `Header` (view switcher + playback cluster, a **gear** → Settings, and `ProfileMenu`), `SettingsPage` (one dedicated view — Voice & reading, Chat & inference *with a per-model selector*, Connection, Appearance, and the **Account** section), `ProfileMenu` (identity, Settings, dark-mode toggle, log out). `AccountPanel` (profile + personal-access-token management) renders **inside** `SettingsPage`; `AdminConsole` is a **top-level view** (`viewMode:'admin'`, admins only): user list, enroll (invite/temp-password), capability editor, activate/disable/delete, last-active-admin guarded.
- **Library**: `LibraryPage` (`viewMode:'library'`, any authenticated active user): list/search documents with project + tag filters, a **New project** form (name + optional description; the reader's project picker refreshes after it), a "shared by …" badge on rows shared with you and a "via project" badge (no remove control) on rows you only reach through a project, **Remove from my library** (removes only your entry) with a confirmation that others/projects keep theirs, and a project-chip selector (add: any project you can see, on docs you hold an upload entry for; remove ×: that project's owner only). A project/tags picker on upload/register; single-document **sharing and adding project members are API-only** until A0's people picker ships (see [LIBRARY.md](LIBRARY.md#sharing)).
- **Chat**: `ChatSidebar` (model picker, sessions, event log, and the "N tokens left today" budget meter — the settings and the **Inference source: Server | Local Ollama** selector moved to `SettingsPage`), `ChatView` (message list with thinking disclosure, markdown rendering, context meter, streaming stop, tool-call and stats disclosures, per-message read-aloud/download; **send disabled at zero budget**), `InferenceSourceSelect` (source toggle; host/port inputs render in local mode only). `AttachmentPreview`; `VoiceRecorder` exists but is intentionally unwired pending Whisper transcription.
...
- **Chrome**: `Header` (playback cluster, view switcher, audiobook export with progress, gear → Settings, `ProfileMenu`), `HeaderOverflowMenu` (mobile), `ViewSwitcher` (Reader / Chat / Library / Admin), `Sidebar` (**navigation only** now — sentence list, chapters; voice/settings/account/admin moved out), `MobileBottomNav`, `DistractionFreeBar`.
...
1. **Logging** (rotating file + console, `logging_config.py`) — plus a second rotating file for the **audit trail** (`server.audit` logger, `LOG_AUDIT_FILE`, default `<LOG_DIR>/audit.log`): one line per change to who holds what (entry added/removed, share created/revoked, placement added/removed, content GC'd), IDs only, never file names or search text.
...
4. **Routers**: TTS, chat sessions, docs, tools, auth, admin, **inference** (the model-router gateway — started/stopped with the app lifecycle, owns one shared httpx client for Ollama).
5. **Startup**: `startup_guard()` (fail fast on auth-bypass or weak secret with a non-loopback bind), `init_db()` (retrying; **failure does not kill the app** — TTS keeps serving, DB-backed routes return 503), embeddings/web-search httpx clients, the document storage dir (`DOC_STORAGE_DIR`, falling back to the pre-A1 `PDF_STORAGE_DIR`, then `./data/pdfs`).
...
- **`docs.py`** — the RAG pipeline plus the **Library** surface, built on the A1 content/entries model (spec: [superpowers/specs/2026-09-25-document-content-entries-design.md](superpowers/specs/2026-09-25-document-content-entries-design.md)): content (`documents`, keyed by `doc_id` = SHA-256 **computed by the server** from the uploaded bytes, regex-gated `^[0-9a-f]{64}$` because it's interpolated into filesystem paths) belongs to nobody; a `library_entries` row ("I have this") belongs to one user, a `project_documents` row belongs to a project. State machine: `stored → extracting → extracted → indexing → indexed | failed` (`registered` is legacy-only, pre-A1 rows with neither bytes nor chunks). `POST /v1/docs` is multipart — the server hashes the bytes itself, a `client_doc_id` is only a hint — and returns 200 (known bytes: instant entry, no rework) or 202 (new bytes: background extract-then-embed job). Extraction (`services/extract.py`) replicates the reader's own page/section-splitting rules exactly, so a chat citation's page number always matches; indexing embeds in batches of 16 under a per-doc lock; search is pgvector cosine distance. Docling conversion (PDF → per-page markdown) lives here too. Read routes honor **`can_read`** (entry holder or member/owner of a project the content is placed in — a **404**, never a 403, for non-readers), resolved by the single predicate in `server/auth/authz.py` (`readable_docs_where` + `readable_docs_params`); content-changing ops (convert, delete converted markdown, re-index an already-`indexed` doc) are refused **409 `content_shared`** unless the caller is the sole holder or an admin. `GET /v1/docs` lists entries plus placements (`q`/project/tag filters); `PATCH /v1/docs/{id}` sets the caller's own `file_name`/tags only — there's no owner to reassign any more; project links go through `projects.py`'s `PUT`/`DELETE /v1/projects/{id}/docs/{doc_id}`. `PUT`/`DELETE /v1/docs/{id}/shares/{user_id}` replace the old per-doc grants. **`POST /v1/docs/{id}/chunks` and `POST`/`DELETE /v1/docs/{id}/pdf` are removed** — bytes arrive once, at registration, and chunks are always server-derived, never client-supplied.
- **`projects.py`** — owner-only project CRUD and membership, plus project↔document links: an **upload**-entry holder files a document in; only the project (A1: its owner; A0: Maintainer+) removes it, so the person who filed it has no special standing over the placement. Backs the Library's sharing model (migrations `009`, `010`, `011`).
...
- **`admin.py`** — list/patch users; disabling hard-revokes their sessions. Also the inference governance endpoints: `GET /v1/admin/inference/usage` (per-day per-user token rows, `days` clamped 1..90) and the per-user `inference_daily_token_budget` field on the PATCH (`exclude_unset` semantics: absent = untouched, explicit null = clear).
- **`inference.py`** — **the model-router gateway** (spec: [superpowers/specs/2026-09-17-model-router-gateway-design.md](superpowers/specs/2026-09-17-model-router-gateway-design.md)). `GET /v1/inference/models` (Ollama tags filtered by the allowlist + the caller's budget state) and `POST /v1/inference/chat` — a strictly validated envelope (`extra="forbid"` down to per-message fields; unknown anything = 422) forwarded to Ollama, **byte-faithful NDJSON streaming** back via `client.send(..., stream=True)` + close in the generator's `finally`. Allowlist violations 422; Ollama down → 502. Budget enforcement lives here: 429 pre-check before the upstream call, and a `_UsageTap` on the final stream chunk that records usage (aborted streams never account) — **fail-open** on DB errors so chat survives a Postgres outage. The SPA reaches Ollama *only* through this when its inference source is Server; the old browser→Ollama path is closed.
- **`endpoints.py`** — TTS + `/v1/health` (see [TTS pipeline](#tts-pipeline)).

### Services

- **`doc_content.py`** — who holds content (spec §3, §5): the only module that writes `library_entries`/`project_documents` changes and the only one allowed to `DELETE FROM documents` — routes never do that directly. Owns the sole-holder check for content-changing ops (`content_ops_refusal`: an admin, or exactly one entry, theirs, and no project placement — otherwise `other_holders`/`in_project`) and `gc_content_if_orphaned`, called after every path that removes an entry or placement, because a SQL `ON DELETE CASCADE` never runs Python. A startup sweep (`db.py`) catches anything a caller forgot to GC.
- **`doc_storage.py`** — resolves `DOC_STORAGE_DIR` (falls back to the pre-A1 `PDF_STORAGE_DIR`, then `./data/pdfs`), enforces `PDF_UPLOAD_MAX_MB`/`TEXT_UPLOAD_MAX_MB` (413), and type-checks by extension + magic bytes (`%PDF-` for PDF, valid UTF-8 for text/markdown; anything else or a NUL byte in text → 415; 0 bytes → 422). Bytes are written to a temp file and moved into place only after the row insert commits, so a concurrent GC can never delete a file a newer upload just wrote.
- **`extract.py`** — server-side text extraction that **replicates the reader's own page/section-splitting rules** (`src/utils/segmentation.js`) exactly, not approximately: PDF via `pypdfium2` (one chunk per non-empty page), text via sentence-splitting grouped 40-per-page, Markdown via a CommonMark+GFM→mdast-equivalent parse (loose lists and tables are one node, never split on blank lines). Pinned by committed parity fixtures (`server/tests/fixtures/segmentation/`) so a citation's page number always matches what the reader jumps to.
- **`embeddings.py`** — Ollama `nomic-embed-text`, **dimension 768 hard-coded** in the SQL column too; swapping embedding models requires dropping the column and re-indexing. Semaphore(4); failed texts return `None` rather than aborting the batch. URL + model come from `model_router` config, not env reads.
- **`model_router.py`** — the single owner of every server-side model choice: `INFERENCE_MODELS` allowlist (unset = all, dev), `SUMMARIZE_MODEL` (falls back to `WEB_SEARCH_SUMMARY_MODEL`, then `llama3.2:3b`), `EMBEDDING_MODEL`, `INFERENCE_TIMEOUT_S`, `INFERENCE_DAILY_TOKEN_BUDGET` (0/unset = unlimited). Pure env parsing; `get_config()` re-reads the environment on every call.
- **`inference_budget.py`** — per-user daily token accounting. Day is computed in **Python, UTC** (`reset_at` = next UTC midnight) — never a SQL default; PK `(user_id, day)` upserts. Per-user column NULL = deployment default, 0 = unlimited. Callers **fail open**: budget errors never block chat.
- **`web_search.py`** — SearXNG query → parallel fetch of results (**SSRF guard**: DNS must resolve to public addresses only, and redirects are followed manually with the check re-applied at every hop, max 4) → trafilatura extraction → small-model summary (model from `model_router`). Degrades to the snippet on any failure.
...
Single `psycopg AsyncConnectionPool`, pgvector codec registered per connection. Pool connections run with `jit=off` (`POOL_CONN_KWARGS`) because the planner overestimates the `GET /v1/docs` row count and Postgres JIT would otherwise compile the query, adding up to ~340 ms per request at scale for a workload that never benefits from it. Migrations are plain `.sql` files in `server/sql/`, applied in numeric order, one transaction each, versioned in `schema_migrations` — no external tooling. The interesting ones: `001` (documents, chunks with `vector(768)` + HNSW cosine index, chat tables), `005` (users + ownership columns + the seed admin row), `006` (sessions + PATs, hashes only), `007` (`inference_usage` PK (user_id, day) — **day computed in Python/UTC, no SQL default** — plus `users.inference_daily_token_budget`), `008` (`users.capabilities` + backfill — the `reader`/`chat`/`admin` realm-role mirror), `009` (`projects`, `project_members`, `doc_grants`, `documents.project_id`/`tags` — the Library access model), `010` (`project_documents` join table — many-to-many projects↔documents, backfilled from and replacing `documents.project_id`), `011` (content vs. entries — drops `doc_grants`/`documents.user_id`/`documents.tags`, renames `pdf_path`→`bytes_path`, adds `documents.extracted_by` and the new `library_entries` table; owners backfill to `upload` entries, grants to `shared` entries; see [LIBRARY.md](LIBRARY.md#upgrade-notes-migration-011)). Restart crash-recovery (`doc_content.recover_states`) rewinds stale `extracting`→`stored` and `indexing`→`extracted` rows and clears a stale `conversion_state='converting'`, then a sweep GCs any content nothing references any more.

---

## End-to-end flows

**Index a document (reader RAG).** Open a PDF → `usePdfEngine` saves bytes to IndexedDB (for the local library/resume list) → `registerDocument` (`src/lib/docMeta.js`) uploads the raw bytes multipart to `POST /v1/docs`, with the client's own sha256 riding along only as a `client_doc_id` hint → the **server** computes the authoritative hash. Known bytes → `200 {dedup: true, state}`, already indexed, nothing more to do. New bytes → `202 {state: "extracting"}`, a background job extracts (server-side, page-parity with the reader) then embeds → poll `GET /v1/docs/{id}` every 2s through `extracting → extracted → indexing → indexed`. Only once `indexed` does `search_document` advertise itself to the chat model.

**Convert with docling.** `registerDocument` first (the server already has the bytes — there is no separate PDF-upload step) → `POST /v1/docs/{id}/convert` with options → backend chains convert → seed chunks from `doc_pages` → embed → poll until `converted` + `indexed` → the reader switches to the markdown view (`MarkdownReader`), fetched via `GET /v1/docs/{id}/markdown` with `<!-- page N -->` markers. Convert, delete-converted-markdown, and re-index (on an already-`indexed` doc) are refused **409 `content_shared`** unless the caller is the document's sole holder (one entry, theirs, no project placement) or an admin — converting changes what every other holder reads.

**One chat turn.** `sendMessage` assembles history (pins as a preamble immediately before the user turn) → `callChat()` sends it through the **inference gateway** (`POST /v1/inference/chat`, default) with tools (if any are active) — or direct to local Ollama in Local mode → stream NDJSON: content, thinking, tool_calls, metrics (nanosecond durations) → if tools were called: execute all in parallel, re-POST history + tool results **without** tools, stream the final answer → meanwhile `chatTtsMode='streaming'` flushes complete sentences to the TTS queue as they form → session auto-creates on first save, title from the prompt. A **429** (budget exhausted) short-circuits before any fallback chain: toast + logged event + aborted turn.
