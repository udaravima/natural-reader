...
- **Library**: `LibraryPage` (`viewMode:'library'`, any authenticated active user): list/search documents with project + tag filters, a "shared by …" badge on rows shared with you and a "via project" badge (no remove control) on rows you only reach through a project, **Remove from my library** (removes only your entry) with a confirmation that others/projects keep theirs, and a project-chip selector (add: any project you can see, on docs you hold an upload entry for; remove ×: that project's owner only). A project/tags picker on upload/register; single-document **sharing is API-only** until A0's people picker ships (see [LIBRARY.md](LIBRARY.md#sharing)).
