import pytest
from fastapi import HTTPException

from server.auth.authz import assert_holds_upload, assert_owns_session
from server.auth.users import resolve_or_provision_user
from server.tests import seed

SEED = "00000000-0000-0000-0000-000000000001"


async def _doc(db_conn, doc_id, holder):
    await seed.seed_doc(db_conn, doc_id, holder, file_name="f", file_type="text")


async def _other(db_conn):
    # First login claims the seed admin; the second is a distinct user.
    await resolve_or_provision_user(db_conn, iss="i", sub="s1", email="admin@x.io")
    other = await resolve_or_provision_user(db_conn, iss="i", sub="s2", email="b@x.io")
    assert other["id"] != SEED
    return other["id"]


async def test_upload_holder_ok(db_conn):
    await _doc(db_conn, "d1", SEED)
    await assert_holds_upload(db_conn, "d1", SEED)  # no raise


async def test_missing_doc_is_404(db_conn):
    with pytest.raises(HTTPException) as e:
        await assert_holds_upload(db_conn, "nope", SEED)
    assert e.value.status_code == 404


async def test_non_holder_is_404_not_403(db_conn):
    other = await _other(db_conn)
    await _doc(db_conn, "d2", SEED)
    with pytest.raises(HTTPException) as e:
        await assert_holds_upload(db_conn, "d2", other)
    assert e.value.status_code == 404


async def test_shared_entry_is_not_an_upload_entry(db_conn):
    # A share recipient holds the content but never proved possession.
    other = await _other(db_conn)
    await _doc(db_conn, "d3", SEED)
    await seed.share_doc(db_conn, "d3", SEED, other)
    with pytest.raises(HTTPException) as e:
        await assert_holds_upload(db_conn, "d3", other)
    assert e.value.status_code == 404


async def test_session_ownership(db_conn):
    await db_conn.execute(
        "INSERT INTO chat_sessions (id, user_id) VALUES ('sess1', %s)", (SEED,)
    )
    await assert_owns_session(db_conn, "sess1", SEED)  # no raise
    with pytest.raises(HTTPException) as e:
        await assert_owns_session(db_conn, "sess1", "00000000-0000-0000-0000-000000000009")
    assert e.value.status_code == 404
